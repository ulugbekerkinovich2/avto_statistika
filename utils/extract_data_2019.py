...
def extract_file_data_2019(file_name, file_id):
    print('2019 data')
    file_id_ = file_id
    from basic_app.models import Model1, Mark, Data19
    df = pd.read_excel(file_name, sheet_name=0)
    data = df.values
    result = Model1.objects.values('model_name', 'mark__mark_name')
    for item in range(len(data)):
        if item >= 2:
            mode = str(data[item][1]).split(' ')[0]
            date = (str(data[item][0])).replace('.', '-')
            product_name_ = data[item][5]
            try:
                product_count = int(data[item][6])
            except:
                if pd.isna(product_count) or product_count == 'nan':
                    product_count = 0
            country = data[item][3]
            cost = data[item][7] * 1000
            # if cost < 1000:
            #     continue
            obj = {
                'mode': mode,
            ...
            if product_count == 0 or cost == 0:
                continue
            # print(cost, product_count, product_name_)
            # time.sleep(1)
            # if (float(str(cost).replace(' ', '')) / float(product_count)) < 1000:
            #     continue
                        ...
                        try:
                            ...
                            if model_instance and mark_instance:
                                ...
                                with transaction.atomic():
                                    data19_instance = Data19.objects.create(
                                        time=datetime.now(),
                                        file_id_id=file_id_,
                                        sana=date_obj,
